...
export const getOwnershipSummary = async (assetId) => {
  const response = await axiosClient.get('/api/ownership/ownerships/summary/', {
    params: { asset_id: assetId}
  });
  return response.data;
};

// Get ownership composition
export const getOwnershipComposition = async (assetId) => {
  const response = await axiosClient.get('/api/ownership/ownerships/composition/', {
    params: { asset_id : assetId }
  });
  return response.data;
};